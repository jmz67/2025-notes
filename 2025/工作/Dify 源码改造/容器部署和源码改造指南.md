...
# 设置国内的 pip 镜像源，使用阿里云的镜像加速
RUN pip config set global.index-url https://mirrors.aliyun.com/pypi/simple/

# sed stream editor 流编辑器，用于对文本进行编辑和转换
# -i 这个选项代表的是原地编辑 in place，即直接修改原始文件，而不是将修改后的内容输出到标准输出
# 's@deb.debian.org@mirrors.aliyun.com@g' 这是 sed 的一个替换命令
...
# 复制 ARM64 Miniconda 安装脚本
COPY Miniconda3-latest-Linux-aarch64.sh /tmp/miniconda.sh
...
ENV CONDA_DEFAULT_ENV=dify-env

# 设置国内的 conda 镜像源，使用阿里云的 Conda 镜像加速
RUN conda config --add channels https://mirrors.aliyun.com/pypi/simple/ \
    && conda config --set show_channel_urls yes

# 从本地复制 Conda 环境
# 假设你已经打包并存储在 ./dify-env.tar.gz 里
COPY ./dify-env.tar.gz /tmp/dify-env.tar.gz

# 解压并设置环境变量
RUN mkdir /opt/conda/envs/dify-env && \
    tar -xzf /tmp/dify-env.tar.gz -C /opt/conda/envs/dify-env && \
    rm /tmp/dify-env.tar.gz

# 激活 Conda 环境，不需要执行 conda init，因为在 Docker 中不需要修改 .bashrc 文件
# RUN conda init \ && conda acitvate difpy 会失效
# conda init 命令，它会修改用户的 ~/.bashrc 文件以配置 Conda 环境，然而 Docker 中的构建是非交互式的，
# 这意味着修改后的 ~/.bashrc 并不会立即生效，从而导致后续的 conda activate dify-env 命令失败。
RUN /bin/bash -c "source /opt/conda/bin/activate dify-env && conda list"
...
# 安装其他依赖
RUN apt-get update \
    && apt-get install -y curl nodejs libgmp-dev libmpfr-dev libmpc-dev \
    && apt-get install -y fonts-noto-cjk \
    && apt-get autoremove -y \
    && rm -rf /var/lib/apt/lists/*

# 设置 Conda 环境
ENV PATH=/opt/conda/envs/dify-env/bin:$PATH
...
SECRET_KEY=I0ymicnz1/gkAFYgvtpSwhEAkbYFsLmDygJK2DMVA51VVuSzq6+8wikD
...
CELERY_BROKER_URL=redis://ningtang@47.99.172.64:26379/0

# redis configuration
REDIS_HOST=47.99.172.64
REDIS_PORT=26379
REDIS_USERNAME=
