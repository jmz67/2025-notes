...
在每个组件中都使用状态不是个太好的主意。状态数据分散在多个组件中不便于追踪 bug 和修改应用，因为你很难确定状态值在组件树中的具体位置。如果集中在一处管理，你对应用的状态或某个功能的状态将有更好地掌握。这个思想很有很多具体做法，我们要分析的第一种做法是在状态树的根部存储状态，通过属性把状态传给子组件。
